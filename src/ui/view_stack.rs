...
pub fn build_view_stack() -> (Box<StackSwitcher>, Box<Stack>, Box<Scale>, Box<ToggleButton>, Box<ToggleButton>, Box<ToggleButton>, Box<DropDown>) {
    ...
    let _page1 = stack.add_titled(speed.as_ref(), Option::<&str>::None, "Speedlol");
    ...
    (Box::from(stack_switcher), Box::from(stack), scale, flash, marquee, invert, drop_down)
}